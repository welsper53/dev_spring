class ImageUploader {
    ...
    async upload(file) {
      ...
      // POST방식으로 전송 시 body에 서버로 전송할 값을 담아줘야 한다
      const data = new FormData(); // 외부 클라우드 시스템과 연동 시 꼭 필요한 객체

      // input type file에서 선택한 파일에 대한 정보 담기
      data.append("file", file);

      // Cloudinay에서 제공하는 서비스를 이용하여 사용자가 선택한 파일에 대한 URL정보만
      // 제공 받아서 처리하므로 upload_preset은 unsigned로 받을 것
      // unsigned - 인증과정 없이 사용 (-> cxqgiiqc)
      data.append("upload_preset", "cxqgiiqc");
